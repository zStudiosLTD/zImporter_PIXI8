...
export class ZScene {
    ...
    loadStage(globalStage: PIXI.Container, loadChildren: boolean = true): void {
        ...
        if (children && loadChildren) {
            for (let i = 0; i < children.length; i++) {
                const child = children[i] as InstanceData;
                if (child.guide) continue;
                const tempName = child.name;
                const mc: ZContainer | undefined = this.spawn(tempName);
                if (mc) {
    ...
    async loadAssets(
        ...
        const _jsonPath: string =
            assetBasePath + "ta.json?rnd=" + Math.random();
        ...
        if (isAtlas) {
            try {
                this.scene = await PIXI.Assets.load(
                    _jsonPath,
                    _updateProgressFnctn
                );
            } catch (err) {
            ...
            // short name while the global PIXI cache stores them under full-path aliases.
            ...
            this.scene = loadResult;
            (this.scene as any).textures = sceneTextures;
        ...
        for (let i = 0; i < placemenisObj.fonts.length; i++) {
            const fontName: string = placemenisObj.fonts[i];
            const fntUrl: string =
                assetBasePath + "bitmapFonts/" + fontName + ".fnt";
            const pngUrl: string =
                assetBasePath + "bitmapFonts/" + fontName + ".png";
            ...
            try {
                ...
                if (!response.ok)
                    throw new Error(
                        `Failed to fetch font XML: ${response.statusText}`
                    );
            ...
            if (!(PIXI.BitmapFont as any).available?.[fontName]) {
                try {
                    // PIXI v8: install takes an options object
                    (PIXI.BitmapFont as any).install({
                        data: xmlString,
                        textures: texture,
                    });
                } catch (err) {
    ...
    private createImagesObject(
        assetBasePath: string,
        obj: SceneData
    ): { alias: string; src: string; _texName: string }[] {
        const images: { alias: string; src: string; _texName: string }[] = [];
        const record: any = {};
        const templates: Record<string, TemplateData> = obj.templates;
        for (const template in templates) {
            const children = templates[template].children;
            for (const child in children) {
                const childObj: BaseAssetData = children[child];
                if (childObj.type === "img" || childObj.type === "9slice") {
                    const imgData: SpriteData = <SpriteData>childObj;
                    if (!record[imgData.name]) {
                        record[imgData.name] = true;
                        let texName: string = imgData.name.endsWith("_9S")
                        ...
                        // Scenes from different pages may share the same file name but
                        // live in different directories — a short alias would collide in
                        // PIXI's global Assets cache; the full path is guaranteed unique.
    ...
    getChildrenFrames(_templateName: string): Record<string, AnimTrackData[]> {
        ...
        if (baseNode && baseNode.children) {
            for (let i = 0; i < baseNode.children.length; i++) {
                const childNode = baseNode.children[i] as InstanceData;
    ...
    async createAsset(mc: ZContainer, baseNode: TemplateData): Promise<void> {
        for (let i = 0; i < baseNode.children.length; i++) {
            const childNode = baseNode.children[i] as BaseAssetData;
            ...
            if (type === "bitmapFontLocked") {
                const textInstanceNode = childNode as BitmapFontLocked;
                if (
                    textInstanceNode.fontName &&
                    (PIXI.BitmapFont as any).available?.[textInstanceNode.fontName as string]
                ) {
                    ...
                    const tf = new PIXI.BitmapText({
                        ...
                        style: {
                            fontFamily:
                                textInstanceNode.fontName as string,
                            align:
                                (textInstanceNode.align as any) || "left",
                        },
                    });

                    if (
                        textInstanceNode.textAnchorX !== undefined &&
                        textInstanceNode.textAnchorY !== undefined
                    ) {
                        tf.anchor.set(
                            textInstanceNode.textAnchorX,
                            textInstanceNode.textAnchorY
                        );
                    }
                    if (
                        textInstanceNode.pivotX !== undefined &&
                        textInstanceNode.pivotY !== undefined
                    ) {
                        tf.pivot.set(
                            textInstanceNode.pivotX,
                            textInstanceNode.pivotY
                        );
                    }
                    tf.name = _name;
                    (mc as any)[_name] = tf;
            ...
            if (type === "bitmapText" || type === "textField") {
                const textInstanceNode = childNode as any as TextData;
                if (
                    textInstanceNode.uniqueFontName &&
                    (PIXI.BitmapFont as any).available?.[
                    textInstanceNode.uniqueFontName as string
                    ]
                ) {
                    // PIXI v8: BitmapText constructor takes an options object
                    const tf = new PIXI.BitmapText({
                        ...
                        style: {
                            fontFamily:
                                textInstanceNode.uniqueFontName as string,
                            fontSize: textInstanceNode.size as number,
                    ...
                    if (
                        textInstanceNode.fillType === "gradient" &&
                        ...
                        const gradient = new FillGradient(
                            ...
                            0,
                            0,
                            Number(textInstanceNode.size) || 16
                        );
                        gradient.addColorStop(
                            textInstanceNode.gradientData.percentages[0],
                            textInstanceNode.gradientData.colors[0]
                        );
                        gradient.addColorStop(
                            textInstanceNode.gradientData.percentages[1],
                            textInstanceNode.gradientData.colors[1]
                        );
                        style.fill = gradient;
                    }
                    ...
                    if (textInstanceNode.dropShadow) {
                        style.dropShadow = {
                            ...
                            distance:
                                textInstanceNode.dropShadowDistance || 0,
                        };
                    }
            ...
            if (type === "img") {
                ...
                const _w: number = spriteData.width;
                const _h: number = spriteData.height;
                const _x: number = spriteData.x || 0;
                const _y: number = spriteData.y || 0;
                const pivotX: number = spriteData.pivotX || 0;
                const pivotY: number = spriteData.pivotY || 0;
                let texName = _name;
                texName = texName.endsWith("_IMG")
                    ? texName.slice(0, -4)
                    : texName;
            ...
            if (type === "9slice") {
                ...
                const _w: number = nineSliceData.width;
                const _h: number = nineSliceData.height;
                const _x: number = nineSliceData.x || 0;
                const _y: number = nineSliceData.y || 0;
                const pivotX: number = nineSliceData.pivotX || 0;
                const pivotY: number = nineSliceData.pivotY || 0;
                let texName = _name;
                texName = texName.endsWith("_9S")
                    ? texName.slice(0, -3)
                    : texName;
                const nineSlice: ZNineSlice = new ZNineSlice(
                    this.scene!.textures[texName],
                    nineSliceData,
                    this.orientation
                );
            ...
            if (ZScene.isAssetType(type)) {
                ...
                if (Object.keys(frames).length > 0) {
                    ...
                    if (
                        ...
                        (asset as ZTimeline).setCuePoints(
                            this.data.cuePoints[childNode.name]
                        );
                    }
                } else {
            ...
            if (type === "spine") {
                ...
                await zSpine.load((spine: Spine | undefined) => {
                    if (spine) {
                        mc.setChilSpineData(spineData);
                        mc.addChild(spine);
                        if (
                            spineData.slotAttachments &&
                            ...
                            for (const attachment of spineData.slotAttachments) {
                                ...
                                const slotContainer = (spine as any)
                                    .slotContainers?.[slotIndex];
                                if (!slotContainer) continue;
                                this.addSlotAttachment(
        ...
        if (obj.filters) {
            for (const k in obj.filters) {
                ...
                if (filter.type === "dropShadow") {
                    ...
                    const _angle: number = filter.rotation || 0;
                    const _dist: number = filter.distance || 4;
                    ...
                    (tf.filters as any[]).push(dropShadowFilter);
                }
                if (filter.type === "blur") {
                    ...
                    (tf.filters as any[]).push(blurFilter);
                }
                if (filter.type === "colorMatrix") {
                    ...
                    (tf.filters as any[]).push(colorMatrixFilter);
                }
            }
        }
    }
    ...
    async createBitmapTextFromXML(
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to fetch XML font data: ${response.statusText}`
            );
        }
        ...
        const textureUrl: string = "./../assets/" + fileAttribute;
    ...
    loadTexture(textureUrl: string): Promise<PIXI.Texture> {
        return PIXI.Assets.load<PIXI.Texture>(textureUrl);
    }
}
