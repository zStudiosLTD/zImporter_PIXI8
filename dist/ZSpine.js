 ...
 */
import { Spine, SpineTexture, TextureAtlas, TextureAtlasPage, TextureAtlasRegion, AtlasAttachmentLoader, SkeletonJson, } from "@esotericsoftware/spine-pixi-v8";
 ...
 */
function addRegion(atlas, page, name) {
 ...
 */
function buildAtlasFromTextures(textures) {
...
export class ZSpine {
    ...
    async load(callback) {
        ...
        if (spineData.spineAtlas && spineData.spineAtlas !== "") {
            try {
                ...
                await Promise.all(atlas.pages.map(async (page) => {
                    const tex = await PIXI.Assets.load(atlasDir + page.name);
        ...
        if (spineData.pngFiles && spineData.pngFiles.length) {
            ...
            await Promise.all(spineData.pngFiles.map((file) => PIXI.Assets.load(assetBasePath + file).then((texture) => {
            ...
            const spineFolderFullPath = assetBasePath +
                spineData.spineJson.substring(0, spineData.spineJson.lastIndexOf("/"));
            const spineFolderName = spineFolderFullPath.substring(spineFolderFullPath.lastIndexOf("/") + 1);
            ...
            for (const skinName of Object.keys(rawSkeletonData.skins ?? {})) {
                ...
                for (const slotName of Object.keys(skin.attachments)) {
                    ...
                    for (const attachmentName of Object.keys(attachmentsForSlot)) {
                        if (atlas.findRegion(attachmentName))
                            continue;
